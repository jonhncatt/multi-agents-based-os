...
| `packages/runtime_core/kernel_host.py` | Legacy host compatibility shell | Agent OS assembly still needs a compatibility host object during legacy-office migration | bootstrap legacy host wiring; explicit legacy host facade; health/role-lab/sandbox surfaces still observe legacy host data; `__getattr__` fallback access is now drained under the full verification pass, but the mixed host object still exists as a class-level compatibility shell | class-level retirement is completed after Agent OS assembly no longer instantiates the compatibility host, `get_legacy_host()` is no longer required by runtime paths, and legacy runtime inspection/compaction surfaces are fully served by canonical facades instead of the legacy host class |
