...
| `packages/runtime_core/kernel_host.py` | Legacy host compatibility shell | Agent OS assembly still needs a compatibility host object during legacy-office migration | bootstrap legacy host wiring; explicit legacy host facade; health/role-lab/sandbox surfaces still observe legacy host data; host-structure, route-helper, and debug/inspection fallback categories are now drained; only a small office-behavior helper tail still transits the mixed host class | blackboard orchestration lives outside the class, entry-condition verification stays green, and the remaining office helper tail is either re-homed behind `OfficeLegacyHelperSurface` or removed before class-level retirement |
