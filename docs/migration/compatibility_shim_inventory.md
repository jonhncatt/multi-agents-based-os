...
| `packages/runtime_core/kernel_host.py` | Legacy host compatibility shell | Agent OS assembly still needs a compatibility host object during legacy-office migration | bootstrap legacy host wiring; `get_legacy_host()` compatibility-only access; health/role-lab/sandbox surfaces still observe legacy host data | `__getattr__` fallback access is fully observed, blackboard orchestration lives outside the class, and Agent OS runtime surfaces fully replace legacy host snapshots/lifecycle hooks |
