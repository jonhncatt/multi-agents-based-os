...
ROLE_AGENT_LAB_PROFILE = ProductProfile(
    ...
    sidebar_hint="保留 Router / Planner / Worker / Reviewer 等角色链路，用于实验多角色编排、hook 和流程策略。",
    kernel_title="共享底盘概览",
    kernel_subtitle="这里仍然使用同一套 runtime-core，但主视图聚焦 role-agent 编排实验，而不是机器人主核面板。",
    role_title="Role-Agent 实验台",
    role_legend="role = 流程岗位；agent = LLM 驱动；processor = 非 LLM；这里用于观察多角色链路和执行分工。",
